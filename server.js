...
const argv = yargs
  .usage('\nSimple OpenID Connect Relying Party (RP)', {
    ...
    hostname: {
      description: 'The publicly addressable hostname this application refers to',
      required: false,
      string: false,
      default: os.hostname()
    }
...
Issuer.discover(argv.issuer).then(function(issuer) {
  var hostname = (process.env.HEROKU_APP_NAME ? process.env.HEROKU_APP_NAME + '.herokuapp.com' : argv.hostname + ':' + argv.port)
  ...
  httpServer.listen(argv.port, function() {
    ...
    const baseUrl   =  scheme + '://' + hostname + ':' + address.port;
